...
\subsection{Multi-Restart as Loss Landscape Navigation}
The dominance of multi-restart training ($r = 0.780$) over SWA ($r = 0.727$) suggests multiple distinct basins of attraction. SWA averages model weights, which is counterproductive when restarts converge to qualitatively different solutions.
...
We presented Universe Multi Neural network (UMN), a differentiable N-body gravitational simulation architecture for olfactory mixture similarity prediction. UMN achieves $r = 0.780$ on the Snitz dataset using 5-seed $\times$ 5-fold cross-validation without manual feature engineering. Key findings: (1) the physics engine architecture is near-optimal in its simplest form, (2) multi-restart training is the most effective strategy, (3) contrastive learning and physics-based losses are counterproductive in small-data regimes, and (4) domain-mismatched augmentation degrades performance.

Limitations include the small dataset size (360 pairs), the absence of identical-protocol comparison with Snitz et al. (2013), and the lack of chemical interpretability of learned physical quantities. Future directions include larger datasets, chemical interpretation of learned quantities, simulation-GNN ensembles, and concentration-aware models.
